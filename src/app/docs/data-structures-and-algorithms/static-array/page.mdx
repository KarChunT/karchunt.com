...
To read from a static array, you can use the index of the element you want to access. T index starts from **0**. Now, accessing the element in a static array is done in **constant time O(1)**, because each **index maps directly to a specific memory location**.

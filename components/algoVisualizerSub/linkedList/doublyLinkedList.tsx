...
const INITIAL_NODES: DoublyListNode[] = [
  {
    value: 42,
    id: '1',
    address: '0x4000',
...
const DoublyLinkedListVisualization = () => {
  const [nodes, setNodes] = useState<DoublyListNode[]>(INITIAL_NODES);
  ...
  const generateAddress = () => {
    ...
    return `0x${(0x4000 + randomOffset).toString(16).toUpperCase()}`;
  };
  ...
  const executeOperation = useCallback(
    (operation: Operation) => {
      ...
      switch (operation.type) {
          ...
          setTimeout(() => {
            setNodes((prev) => {
              ...
              return newNodes.map((node, idx) => ({
                ...
                prevAddress: idx > 0 ? newNodes[idx - 1].address : null,
              }));
            });
          }, 500);
          break;
        ...
        case 'insertBegin':
          const newBeginNode: DoublyListNode = {
          ...
          setNodes((prev) => {
            const updated = [newBeginNode, ...prev];
            if (updated.length > 1) {
              updated[1].prevAddress = newBeginNode.address;
            }
        ...
        case 'insertEnd':
          const newEndNode: DoublyListNode = {
            value: operation.value,
            id: Date.now().toString(),
            address: generateAddress(),
            nextAddress: null,
        ...
        case 'insert':
          const newNode: DoublyListNode = {
            ...
            nextAddress:
              operation.index < nodes.length
                ...
                : null,
            prevAddress:
              operation.index > 0 ? nodes[operation.index - 1].address : null,
            isNew: true,
          };
          ...
          setNodes((prev) => {
            ...
            if (operation.index < newNodes.length - 1) {
              newNodes[operation.index + 1].prevAddress = newNode.address;
            }
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <span>Doubly Linked List Visualization</span>
            {currentOperation && (
            ...
            {nodes.map((node, index) => (
              <div key={node.id} className="flex items-center gap-2">
                <div className="flex flex-col items-center gap-1">
                  ...
                  <div
                    className={`flex h-28 w-32 transform flex-col items-center justify-center rounded-lg border-2 font-mono transition-all duration-500 ${node.isHighlighted ? 'border-chart-2 bg-chart-2/20 scale-110' : ''} ${node.isActive ? 'border-primary bg-primary/20 scale-110' : ''} ${node.isDeleted ? 'border-destructive bg-destructive/20 scale-75 opacity-50' : ''} ${node.isNew ? 'border-chart-3 bg-chart-3/20 scale-110' : ''} ${!node.isHighlighted && !node.isActive && !node.isDeleted && !node.isNew ? 'border-border bg-card' : ''} `}
                  >
                    <div className="mb-1 text-lg font-bold">{node.value}</div>
                    <div className="text-muted-foreground text-xs">
                      prev: {node.prevAddress || 'null'}
                    </div>
                    <div className="text-muted-foreground text-xs">
